...
function s.initial_effect(c)
	...
	e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0xa008))
	e2:SetValue(500)
	c:RegisterEffect(e2)
	--atkup Kaijin
	local e2=Effect.CreateEffect(c)
...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(eg) do
		if tc:IsReason(REASON_DESTROY) and tc:IsSetCard(0xa008) and not tc:IsPreviousLocation(LOCATION_SZONE) then
...
function s.spfilter(c,e,tp,lv)
	return c:IsLevelBelow(lv) and c:IsSetCard(0xa008) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
